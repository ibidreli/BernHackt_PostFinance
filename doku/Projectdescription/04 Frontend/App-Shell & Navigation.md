...
- Routen (`app.routes.ts`, alle auf `main`): `''` → DashboardLayout mit lazy geladenem [[Kategorien-Explorer]] (seit PR #13, ersetzt das frühere Dashboard), `/forecast` ([[Forecast-Seite]]) und `/assistant` ([[Assistant-Seite]]), plus `core/rail.ts` (rechte Kontextspalte via `ng-template` + Directive) und `core/chart-theme.ts` (`chf()`-Formatter). Ziel-Reihenfolge und Routen-Tausch: [[Sollstatus]].
