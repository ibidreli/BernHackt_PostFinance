...
- Routen (`app.routes.ts`): `''` → DashboardLayout mit lazy geladenem Dashboard; auf dem Branch `feature/prognosis` zusätzlich `/forecast` ([[Forecast-Seite]]) und `/assistant` ([[Assistant-Seite]]), plus `core/rail.ts` (rechte Kontextspalte via `ng-template` + Directive) und `core/chart-theme.ts` (`chf()`-Formatter).
