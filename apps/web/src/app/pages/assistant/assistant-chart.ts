...
export class AssistantChart {
  ...
  private config(spec: ChartSpec): ChartConfiguration<'line'> {
    ...
    const large = Math.max(...spec.series.map((p) => Math.abs(p.expected_chf))) >= 5000;
    ...
    const expected: ChartDataset<'line'> = {
      ...base,
      label: 'Erwartet',
      data: spec.series.map((p) => p.expected_chf),
      borderColor: line,
    ...
    const band: ChartDataset<'line'>[] = [
      { ...base, label: 'Optimistisch', data: spec.series.map((p) => p.upper_chf), borderWidth: 0, fill: '+1', backgroundColor: `color-mix(in srgb, ${line} 10%, transparent)` },
      { ...base, label: 'Pessimistisch', data: spec.series.map((p) => p.lower_chf), borderWidth: 0, fill: false },
    ];
    const target: ChartDataset<'line'>[] =
      spec.target_line_chf === null
        ? []
        : [{ ...base, label: 'Ziel', data: spec.series.map(() => spec.target_line_chf!), borderColor: accent, borderDash: [6, 5], borderWidth: 1.5, fill: false }];
    const baseline: ChartDataset<'line'>[] = spec.series[0]?.baseline_chf === null
      ? []
      : [{ ...base, label: 'Ohne Änderung', data: spec.series.map((p) => p.baseline_chf ?? 0), borderColor: muted, borderDash: [5, 4], fill: false }];
    ...
    return {
      type: 'line' as const,
      data: { labels: spec.series.map((p) => shortLabel(p.date)), datasets: datasets[spec.type] },
      options: {
        responsive: true,
